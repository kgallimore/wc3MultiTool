...
export function generateAutoBalance(
  ...
  if (teams.length < 2) {
    ...
    let players = Object.entries(playerData).filter(
    ...
    for (const combo of combos) {
      const comboElo = combo.reduce(
        (a, b) => a + ensureInt(playerData[b].extra?.rating ?? 500),
        0
      );
    ...
    let lobbyCopy = new MicroLobby({ fullData }, false);
    ...
    for (let i = 0; i < bestCombo.length; i++) {
      ...
      for (let x = 0; x < bestCombo[i].length; x++) {
        ...
        let targetPlayer = bestCombo[i][x];
