...
module.exports = {
  ...
  async execute(interaction: Interaction<"cached">, channelMatch: ChatChannelMatch) {
    ...
    if (command === "uploadmap") {
      ...
      if (
        !["w3x", "w3m"].includes(file.name.split(".")[file.name.split(".").length - 1])
      ) {
        await interaction.editReply({
      ...
      if (key) {
        ...
        let clean = settings.getCleanSetting(setting);
        let build = Object.entries(clean).map(([key, value]) => {
          return { name: key, value, inline: true };
        });
        let embed = new EmbedBuilder().setTitle(setting).addFields(build).setTimestamp();
        await interaction.reply({
          embeds: [embed],
          ephemeral: channelMatch !== "admin",
        });
