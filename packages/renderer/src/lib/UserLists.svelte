<script lang="ts">
  ...
  import {type BanWhiteList, type WindowSend, commandArray} from './../../../main/src/utility';
    ...
    <div class="row justify-content-center p-2">
      <div class="col d-flex text-xs">
        <details>
          ...
          {#each commandArray.filter(([, details]) => details.minPermissions === 'moderator') as [command, details]}
            ?{command}{details.arguments ? ' ' + details.arguments : ''}: {details.description}<br
            />
          {/each}
          <strong>Admin:</strong><br />
          {#each commandArray.filter(([, details]) => details.minPermissions === 'admin') as [command, details]}
            ?{command}{details.arguments ? ' ' + details.arguments : ''}: {details.description}<br
            />
          {/each}
        </details>
