...
class ProtocolHandler extends ModuleBase {
  ...
  async processURL(url: string) {
    if (url) {
      this.gameState.values.openLobbyParams = this.getQueryVariables(url.split('?', 2)[1]);
      if (
        this.gameState.values.openLobbyParams.lobbyName ||
        this.gameState.values.openLobbyParams.gameId
      ) {
        ...
        if (await this.warControl.isWarcraftOpen()) {
          if (
            ...
            this.gameState.values.openLobbyParams.region !== this.gameState.values.selfRegion
          ) {
            this.info(`Changing region to ${this.gameState.values.openLobbyParams.region}`);
          ...
          this.info(
            'Warcraft is not open, opening. ' + this.gameState.values.openLobbyParams.region
              ? this.gameState.values.openLobbyParams.region
              : '',
          );
          try {
            await this.warControl.openWarcraft(this.gameState.values.openLobbyParams.region);
          } catch (e) {
            this.warn(e);
          }
        }
