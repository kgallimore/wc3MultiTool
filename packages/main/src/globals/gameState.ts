...
import type {GameSocketEvents} from './gameSocket';
...
class GameStateContainer extends Global {
  ...
  onGameSocketEvent(events: GameSocketEvents) {
    ...
    if ((events.GameList?.games && events.GameList.games.length > 0) || events.GameListClear) {
  ...
  updateGameState(values: Partial<GameState>) {
    ...
    (
      Object.entries(values) as {
        [K in keyof GameState]: [keyof PickByValue<GameState, GameState[K]>, GameState[K]];
      }[keyof GameState][]
    ).forEach(([key, value]) => {
