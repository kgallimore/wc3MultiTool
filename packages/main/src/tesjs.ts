...
import type {PlayerTeamsData, PlayerData} from 'wc3mt-lobby-container';
const playerTeamsData: PlayerTeamsData = {
  team1: [
    {
      name: 'suken',
      slotStatus: 1,
...
const autoBalance = generateAutoBalance(playerTeamsData, nonSpecPlayers, false, false);
if (autoBalance == true) {
  console.log('Already balanced');
} else {
  if (autoBalance.twoTeams) {
    const bestComboElo = autoBalance.twoTeams.bestCombo.reduce(
